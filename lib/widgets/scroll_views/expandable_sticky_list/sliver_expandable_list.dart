...
typedef ExpandableSectionBuilder<S> = Widget Function(
  BuildContext context,
...
class SliverExpandableList<S> extends SliverList {
  ...
  void updateRenderObject(
    BuildContext context,
    RenderExpandableSliverList renderObject,
  ) {
  ...
  List<bool> _buildExpandStateList() =>
      builder.sectionList.map((e) => e.isSectionExpanded()).toList();
}
...
class SliverExpandableChildDelegate<T, S extends ExpandableListSection<T>> {
  ...
  ///
  ExpandableSectionBuilder<S>? sectionBuilder;
  ...
  SliverExpandableChildDelegate({
    ...
    this.controller,
    @Deprecated("Isn't working properly, todo: check realization")
        this.separatorBuilder,
    this.headerBuilder,
    this.sectionBuilder,
    this.sticky = true,
    this.overlapsContent = false,
    @Deprecated('unused property') this.removeItemsOnCollapsed = true,
    ...
    bool addSemanticIndexes = true,
        ...
        sectionRealIndexes = _buildSectionRealIndexes<T, S>(sectionList) {
    ...
    if (separatorBuilder == null) {
      delegate = SliverChildBuilderDelegate(
        (BuildContext context, int index) {
          ...
          final int sectionChildCount = section.getItems()?.length ?? 0;
          ...
          // call for prefer_null_aware_method_calls
          Widget? container = sectionBuilder?.call(
            context,
            containerInfo,
            sectionIndex,
            section,
          ...
          if (container == null) {
            containerInfo
              ...
              ..content = buildDefaultColumn(context, containerInfo);
      ...
      delegate = SliverChildBuilderDelegate(
        (BuildContext context, int index) {
          ...
          if (index.isEven) {
            ...
            final childBuilderDelegate = SliverChildBuilderDelegate(
              (context, i) {
                final int itemRealIndex = sectionRealIndex + (i ~/ 2) + 1;
                if (i.isEven) {
                  return itemBuilder(
                    context,
                    sectionIndex,
  ...
  static Widget buildDefaultColumn(
    BuildContext context,
    ExpandableSectionContainerInfo containerInfo,
  ) {
  ...
  static List<int>
    ...
    final int calcLength = sectionList.length - 1;
    final List<int> sectionRealIndexes = List<int>.empty(growable: true);
    if (calcLength < 0) {
