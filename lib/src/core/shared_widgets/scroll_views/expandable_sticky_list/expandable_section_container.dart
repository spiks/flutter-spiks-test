...
import 'sliver_expandable_list.dart';
...
class ExpandableSectionContainer extends MultiChildRenderObjectWidget {
  ...
  void updateRenderObject(
    ...
    RenderExpandableSectionContainer renderObject,
  ) {
    renderObject
      ..scrollable = Scrollable.of(context)..controller = info.controller
      ..sticky = info.sticky
      ..overlapsContent = info.overlapsContent
      ..listIndex = info.listIndex
      ..sectionRealIndexes = info.sectionRealIndexes
...
//layout two child
    final BoxConstraints exactlyConstraints = constraints.loosen();
...
//header's size should not large than content's size.
    final double headerLogicalExtent = _overlapsContent ? 0 : header.size.height;
...
//calc content offset
    ...
    final double sliverListOffset = _getSliverListVisibleScrollOffset();
    ...
    final bool containerPainted = (_listIndex == 0 && currContainerOffset == 0) ||
        currContainerOffset > 0;
    if (!containerPainted) {
      positionChild(header, Offset.zero);
      return;
    }
    final double minScrollOffset = _listIndex >= _controller.containerOffsets.length
        ? 0
        : _controller.containerOffsets[_listIndex]!;
    final double maxScrollOffset = minScrollOffset + size.height;
...
// in [containerOffsets].
    if (_listIndex < _controller.containerOffsets.length) {
      currContainerOffset = _controller.containerOffsets[_listIndex]!;
      final int nextListIndex = _listIndex + 1;
...
//calc header offset
    ...
    final double headerMaxOffset = height - header.size.height;
...
// print("visitChildren $containerParentData");
  ...
  void checkRefreshContainerOffset() {
    final int length = _controller.containerOffsets.length;
    ...
    for (int i = 0; i < _listIndex && _listIndex < length - 1; i++) {
      final double currOffset = _controller.containerOffsets[i] ?? 0;
      final double nextOffset = _controller.containerOffsets[i + 1] ?? 0;
